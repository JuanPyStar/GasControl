...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $suministro->fecha_recepcion = date('Y-m-d H:i:s');
    $suministro->proveedor = $_POST['proveedor'] ?? '';
    
    // Create the suministro
    if ($suministro->create()) {
        $message = "Suministro registrado exitosamente y stock actualizado.";
        $success = true;
    } else {
        $message = "No se pudo registrar el suministro.";
    }
}
...
<!-- Agregar Suministro Content -->
<div class="fade-in">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Registrar Nuevo Suministro</h1>
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="cantidad_recibida" id="cantidad_recibida" step="0.01" min="0.01" required
                           class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                    <i class="fas fa-save mr-2"></i> Registrar Suministro
                </button>
            </div>
        </form>
    </div>
